...
# ============================================
# 1) Exakte Eigenwerte (nur für Referenz/Tail-Konstruktion)
# ============================================
...
class RigorousHeatTraceZeta:
    ...
    def zeta(self, s):
        ...
        # geschlossene Anteile (aus (0,1) analytisch)
...
class RigorousEquivalenceDiagnostics:
    """
    A1': Residuum bei s=1/2 (korrekt für 1D) — symmetrische Probe + Richardson.
    ...
    def test_A1_residue_at_half(self, eps0='1e-4', levels=5):
        """A1': Residuum bei s=1/2, symmetrische Probe + Richardson-Extrapolation."""
        print("🔍 Test A1': Residuum bei s=1/2 (Laurent-Fit, symmetrisch + Richardson)...")
        s0 = mp.mpf('0.5')
        eps0 = mp.mpf(eps0)

        def resid_sym(delta):
            d = mp.mpf(delta)
            # symmetrische Schätzung: ½[ (+d)ζ(s0+d) + (−d)ζ(s0−d) ]
            return mp.mpf('0.5') * ( (+d) * self.zeta_H.zeta(s0 + d)
                                     + (-d) * self.zeta_H.zeta(s0 - d) )

        # Folge δ, δ/2, δ/4, ...
        deltas = [eps0 / (2**j) for j in range(levels)]
        row = [resid_sym(d) for d in deltas]

        # Richardson-Extrapolation (Ordnung ~2)
        table = [row]
        for m in range(1, levels):
            prev = table[-1]
            curr = []
            factor = mp.mpf(2)**(2*m)  # Fehler ~ c * δ^(2m)
            for i in range(len(prev) - 1):
                curr.append(prev[i+1] + (prev[i+1] - prev[i]) / (factor - 1))
            table.append(curr)

        residue_fit = table[-1][0]
...
def run_rigorous_diagnostics():
    ...
    # Tests
    for test in [
        diag.test_A1_residue_at_half,           # jetzt symmetrisch + Richardson
...
if __name__ == "__main__":
    ...
    print("   A2' ist korrekt (Xi_H0 mit c^(1/2 - 2s)); A1' nutzt jetzt Richardson.")
