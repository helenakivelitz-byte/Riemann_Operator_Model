...
def heat_trace_from_eigs(eigs, t):
    ...
    terms = [ mp.e**(-t_mp * lam) for lam in eigs ]
...
class HeatTraceZeta:
    ...
    def demo(self, s_points=(0.6, 1.0, 2.0)):
        ...
        print(f"  Intervall: [1, {float(self.L):.1f}]  |  M={self.M}  |  λ_min≈{float(self.eigs[0]):.6f}  |  T={float(self.T_large):.1f}")
        ...
        for s in s_points:
            ...
            # Umwandlung in float für Formatierung
            z_f = float(z)
            z_ref_f = float(z_ref)
...
# Nur diese Parameter ändern:
    HT = HeatTraceZeta(
        L=100.0,      # ← VIEL größeres L
        M=2000,       # ← Mehr Eigenwerte  
        T_large=50.0, # ← Längeres Integral
        t0=1e-8,      # ← Stabileres t0
